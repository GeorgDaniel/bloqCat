...
class TopologyView(MethodView):
    ...
    def post(self):
        ...
        solution_nodes, solution_relationships = self.create_solution_path(nodes, relationships)
        ...
        file_content = self.aggregate_concrete_solution_files(concrete_solution_files, solution_nodes, solution_relationships)
    ...
    def validate_data(self, data):
        ...
        if 'nodeTemplates' not in data or 'relationshipTemplates' not in data:
            return False, "Daten müssen 'nodeTemplates' und 'relationshipTemplates' enthalten."
        ...
        if not any(node.get('name', '').startswith("Concrete Solution of") for node in data.get('nodeTemplates', [])):
    ...
    def validate_path(self, nodes, relationships):
        ...
        concrete_solution_nodes = {node['id']: node for node in nodes if node.get('name', '').startswith("Concrete Solution of")}
        ...
        for relationship in valid_relationships:
            ...
            source_qubits = source_node['properties'].get('kvproperties', {}).get('QubitCount')
            target_qubits = target_node['properties'].get('kvproperties', {}).get('QubitCount')

            # Überprüfen, ob die Qubit-Anzahlen gleich sind
            if source_qubits != target_qubits:
                return False, "Nicht übereinstimmende Qubit-Anzahlen in einer Aggregations-Beziehung."
    ...
    def create_solution_path(self, nodes, relationships):
        ...
        solution_nodes = {node['id']: node for node in nodes if node.get('name', '').startswith("Concrete Solution of")}
        
        # Filtern der Relationships vom Typ 'Aggregation'
        aggregation_relationships = [r for r in relationships if r.get('name') == 'Aggregation']
        ...
        solution_relationships = [
            r for r in relationships 
            if r['sourceElement']['ref'] in solution_nodes and r['targetElement']['ref'] in solution_nodes
        ]
    ...
    def aggregate_concrete_solution_files(self, concrete_solution_files, solution_nodes, solution_relationships):
        # Hier wird der Inhalt der Datei basierend auf den Daten erstellt
        start_pattern_name = solution_nodes[list(solution_nodes.keys())[0]]['name'].replace("Concrete Solution of ", "")
        reg_size = solution_nodes[list(solution_nodes.keys())[0]]['properties'].get('kvproperties', {}).get('QubitCount')

        

        has_header = solution_nodes[list(solution_nodes.keys())[0]]['properties'].get('kvproperties', {}).get('hasHeader')

        file_content = f"// -- Start HEADER --\n"

        if has_header == 'true':
            file_content += f"// -- HEADER created from Pattern \"{start_pattern_name}\" --\n"
